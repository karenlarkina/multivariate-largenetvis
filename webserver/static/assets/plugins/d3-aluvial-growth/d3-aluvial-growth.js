d3.alluvialGrowth = function() {
  ...
  alluvialGrowth.numberTimes = function(_) {
    if (!arguments.length) return numberTimes;
    numberTimes = +_;
    return alluvialGrowth;
  };
  ...
  //Jean tests - drawing sublayers
  alluvialGrowth.drawSublayer = function(link) {
    console.log(link);
...
*/
  ...
  function computeNodeBreadthsNew() {

    nodes.forEach(function(node) {
      node.x = node.time;
      node.dx = nodeWidth;
    });
    //How far the timestamps will be from each other on x-axys
    scaleNodeBreadths((size[0] - nodeWidth) / (numberTimes-1));
